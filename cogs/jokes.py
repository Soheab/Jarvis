...
class ComedicCog(commands.Cog):
    ...
    async def legocity(self, ctx, arg1, arg2):
        await ctx.send(f'{arg1} has fallen into {arg2} in Lego City! **Hey!**')
    ...
    async def sending(self, ctx):
        ...
        await ctx.send(
            'please kids, when you are giving a hand job to george clooney, '
            'remember to stick you finger in his mouth. that gets him hard instantly')
    ...
    @commands.command()
    async def dongsize(self, ctx, member: discord.Member = None):
        sizes = ['8D',
                 '8=D',
                 '8==D',
                 '8===D',
